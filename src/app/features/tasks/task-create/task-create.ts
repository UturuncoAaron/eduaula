...
export class TaskCreate implements OnInit {
  ...
  submit() {
    ...
    if (!isInteractiva && !v.permite_archivo && !v.permite_texto) {
      this.snack.open('Selecciona al menos un método de entrega (archivo o texto)', 'OK', { duration: 3500 });
      return;
    }
  ...
  private uploadReferenceFile(taskId: string, file: File) {
    ...
    this.taskSvc.uploadEnunciado(taskId, file).subscribe({
      ...
      error: () => {
        this.uploading.set(false);
        this.snack.open('Tarea creada, pero falló la subida del archivo de referencia', 'OK', { duration: 4000 });
        this.finishSuccess();
      },
    });
  }
