...
import {
  AppointmentFormDialog, AppointmentFormDialogData,
...
export class TabCitas {
  // PsychologyStore solo para directorio (searchAllStudents etc.)
  protected readonly store = inject(PsychologyStore);
  protected readonly appointmentsStore = inject(AppointmentsStore);
  ...
  readonly visible = computed<Appointment[]>(() => {
    const all = this.appointmentsStore.appointments();
  ...
  readonly stats = computed(() => {
    ...
    return {
      ...
      hoy: all.filter(a => new Date(a.scheduledAt).toDateString() === today).length,
  ...
  constructor() {
    void this.appointmentsStore.loadMyAppointments();
  ...
  openCreate(): void {
    const ref = this.dialog.open<AppointmentFormDialog, AppointmentFormDialogData, boolean>(
    ...
    ref.afterClosed().subscribe(ok => {
      if (ok) void this.appointmentsStore.loadMyAppointments();
    });
  }
  ...
  async setEstado(row: Appointment, estado: AppointmentEstado): Promise<void> {
    ...
    try {
      await this.appointmentsStore.updateAppointment(row.id, { estado });
  ...
  async cancelar(row: Appointment): Promise<void> {
    const ref = this.dialog.open<ConfirmDialog, ConfirmData, boolean>(ConfirmDialog, {
      ...
      data: {
        ...
        confirm: 'Sí, cancelar', cancel: 'Volver', danger: true,
      },
    });
