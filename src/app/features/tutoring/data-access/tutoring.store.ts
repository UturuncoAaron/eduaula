...
import type { TutoriaData, AlumnoTutoria, PadreTutoria, NotebookItem } from './tutoring.types';
...
export class TutoringStore {
    ...
    // ── Progreso libretas de padres ───────────────────────────────────────────
    readonly progresoPadre = computed(() => {
        const pid = this.periodoSeleccionadoId();
        const padres = this.data()?.padres ?? [];
        if (!pid) return { cargadas: 0, total: padres.length };
        const cargadas = padres.filter(p =>
            p.libretas?.some(l => l.periodo_id === pid),
        ).length;
        return { cargadas, total: padres.length };
    });
    ...
    async load(force = false): Promise<void> {
        ...
        try {
            ...
            if (d?.padres) {
                d.padres = d.padres.map(p => ({ ...p, libretas: p.libretas ?? [] }));
            }
            this.data.set(d);
            if (d?.periodo_activo) {
    ...
    libretaDePadre(
        ...
        if (!periodoId) return null;
        return p.libretas?.find(l => l.periodo_id === periodoId) ?? null;
    }
}